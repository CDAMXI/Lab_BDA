...
WHERE M.LENGTH = (SELECT MAX(M2.LENGTH) FROM MOVIE M2)
ORDER BY G.NAME;
...
FROM MOVIE M
WHERE (SELECT SUM(M2.LENGTH)
       FROM MOVIE M2
       WHERE M2.DIRECTOR = M.DIRECTOR) > 250;
...
SELECT M.MOVIE_CODE, M.TITLE
FROM MOVIE M
WHERE M.LENGTH < 100 AND EXISTS(SELECT *
                                FROM COUNTRY C
                                WHERE M.MOVIE_CODE = P.MOVIE_CODE
                                AND A.COUNTRY != C.COUNTRY_CODE)
AND EXISTS (SELECT *
            FROM PERFORMS P
            WHERE P.MOVIE_CODE = M.MOVIE_CODE);
...
SELECT B.BOOK_CODE, B.TITLE, COUNT(M.MOVIE_CODE)
FROM BOOK_MOVIE B JOIN MOVIE M ON B.BOOK_CODE = M.BOOK_CODE
GROUP BY B.BOOK_CODE, B.TITLE
HAVING COUNT(MOVIE_CODE) >= 2
ORDER BY COUNT(M.MOVIE_CODE) DESC;

--Exercise 40

SELECT G.GEN_CODE, G.NAME, COUNT(*) AS "HOW MANY", ROUND(AVG(M.LENGTH)) AS "AVG LENGTH"--COUNT(C.movie_code) IS THE SAME THING
FROM GENRE G JOIN CLASSIFICATION C ON G.GEN_CODE = C.GEN_CODE
              JOIN MOVIE M ON M.MOVIE_CODE = C.MOVIE_CODE
GROUP BY G.GEN_CODE, G.NAME
HAVING COUNT(*) > 5
ORDER BY G.NAME;

--Exercise 41

SELECT M.MOVIE_CODE, M.TITLE, COUNT(*) AS "HOW MANY GENRES"
FROM MOVIE M JOIN CLASSIFICATION C ON M.MOVIE_CODE = C.MOVIE_CODE
WHERE M.YEAR > 2000
GROUP BY M.MOVIE_CODE, M.TITLE
ORDER BY M.TITLE;
...
HAVING COUNT(*) = (SELECT MAX(COUNT(*))
                   FROM MOVIE M2
                   WHERE M2.BOOK_CODE IS NOT NULL
                   GROUP BY M2.BOOK_CODE);

--Exercise 50

SELECT C.COUNTRY_CODE, C.NAME, (SELECT COUNT(*)
                                FROM ACTOR_E A
                                WHERE C.COUNTRY_CODE = A.COUNTRY_CODE)
FROM COUNTRY C
ORDER BY C.NAME;

SELECT C.COUNTRY_CODE, C.NAME, COUNT(A.ACT_CODE) AS ACTORS
FROM COUNTRY C LEFT JOIN ACTOR_E A ON A.COUNTRY_CODE = C.COUNTRY_CODE
GROUP BY C.COUNTRY_CODE, C.NAME
ORDER BY C.NAME;

--Exercise 51

SELECT B.BOOK_CODE, B.TITLE, COUNT(M.BOOK_CODE)
FROM BOOK_MOVIE B LEFT JOIN MOVIE M ON B.BOOK_CODE = M.BOOK_CODE
WHERE B.YEAR > 1980
GROUP BY B.BOOK_CODE, B.TITLE;

--Exercise 52

SELECT C.COUNTRY_CODE, C.NAME, COUNT(A.ACT_CODE)
FROM COUNTRY C LEFT JOIN ACTOR_E A ON C.COUNTRY_CODE = A.COUNTRY_CODE
AND EXISTS (SELECT *
            FROM PERFORMS P
            WHERE P.ROLE = 'Secundario' AND P.ACT_CODE = A.ACT_CODE)
GROUP BY C.COUNTRY_CODE, C.NAME
ORDER BY C.NAME;
